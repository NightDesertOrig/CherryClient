...
public class RD implements Util
{
    ...
    public static float drawString(final String str, final float x, final float y, final int color, final boolean shadow, final float s) {
        final CFontRenderer font = Cherry.m_font.fontRenderer;
        if (shadow) {
            font.drawString(str, x + 0.5 * s, y + 0.5 * s, ColorUtil.toRGBA(new Color(0, 0, 0, 255)), true, 30.0f);
        }
    ...
    public static float guiToScreen(final float shit) {
        final ScaledResolution scaledresolution = new ScaledResolution(RD.mc);
    ...
    public static void Scissor(final float x, final float y, final float width, final float height) {
        RD._lasted = new Rectangle((int)x, (int)y, (int)width, (int)height);
        GL11.glScissor((int)guiToScreen(x), (int)(RD.mc.displayHeight - (guiToScreen(y) + guiToScreen(height))), (int)guiToScreen(width), (int)guiToScreen(height));
    }
    
    public static void pushScissor() {
        RD.lasted = RD._lasted;
    }
    
    public static void popScissor() {
        GL11.glScissor((int)guiToScreen((float)RD.lasted.x), (int)(RD.mc.displayHeight - (guiToScreen((float)RD.lasted.y) + guiToScreen((float)RD.lasted.height))), (int)guiToScreen((float)RD.lasted.width), (int)guiToScreen((float)RD.lasted.height));
    }
    
    static {
        RD.camera = (ICamera)new Frustum();
    }
}
