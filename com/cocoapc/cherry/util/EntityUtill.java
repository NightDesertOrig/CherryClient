...
{
    public static String IlllllIllllIlIll;
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtill.mc.player) || (entity instanceof EntityPlayer && Cherry.friendManager.isFriend(entity.getName())) || EntityUtill.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static EntityPlayer getTargetDouble(final double range) {
        ...
        for (int size = EntityUtill.mc.world.playerEntities.size(), i = 0; i < size; ++i) {
            final EntityPlayer player = EntityUtill.mc.world.playerEntities.get(i);
            if (!EntityUtil.isntValid((Entity)player, range)) {
                if (currentTarget == null) {
                    currentTarget = player;
                }
                else if (EntityUtill.mc.player.getDistanceSq((Entity)player) < EntityUtill.mc.player.getDistanceSq((Entity)currentTarget)) {
                    currentTarget = player;
                }
            }
        }
    ...
    public static EntityLivingBase getTarget(final boolean players, final boolean neutral, final boolean friends, final boolean hostile, final boolean passive, final double range, final int mode) {
        ...
        if (mode == 0) {
            entity2 = (EntityLivingBase)EntityUtill.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).min(Comparator.comparing(entity1 -> EntityUtill.mc.player.getPositionVector().squareDistanceTo(entity1.getPositionVector()))).orElse(null);
        }
        else if (mode == 1) {
            entity2 = (EntityLivingBase)EntityUtill.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).map(entity1 -> entity1).min(Comparator.comparing(entity1 -> EntityUtill.mc.player.getPositionVector().squareDistanceTo(entity1.getPositionVector()))).orElse(null);
        }
        else if (mode == 2) {
            entity2 = (EntityLivingBase)EntityUtill.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).map(entity1 -> entity1).max(Comparator.comparing(entity1 -> EntityUtill.mc.player.getPositionVector().squareDistanceTo(entity1.getPositionVector()))).orElse(null);
        }
        return entity2;
    }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            if (player.isInvisible() && player.getName().equals(EntityUtill.mc.player.getName())) {
                continue;
            }
            ...
            healthSB.append(hp);
            final int distanceInt = (int)EntityUtill.mc.player.getDistance((Entity)player);
    ...
    static {
        EntityUtill.IlllllIllllIlIll = "com";
