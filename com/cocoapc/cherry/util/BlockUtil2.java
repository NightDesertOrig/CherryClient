...
public class BlockUtil2 implements Util
{
    ...
    public static void placeBlockss(final BlockPos blockPos, final boolean bl, final boolean bl2, final boolean bl3) {
        for (final EnumFacing enumFacing : EnumFacing.values()) {
            ...
            if (!BlockUtil2.mc.world.getBlockState(blockPos2.offset(enumFacing)).getBlock().equals(Blocks.AIR) && !isIntercepted(blockPos2)) {
                if (packet) {
    ...
    public static boolean canBlockBeSeen(final double x, final double y, final double z) {
        return BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), new Vec3d(x, y + 1.7, z), false, true, false) == null;
    }
    ...
    public static int isPositionPlaceable(final BlockPos pos, final boolean rayTrace, final boolean entityCheck) {
        ...
        if (entityCheck) {
            for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(pos))) {
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange) {
        final NonNullList positions = NonNullList.create();
        positions.addAll((Collection)BlockUtil.getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtil2.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(BlockUtil::canPlaceCrystal).collect(Collectors.toList()));
        return (List<BlockPos>)positions;
    }
    ...
    public static boolean canPlaceCrystal(final BlockPos blockPos, final boolean specialEntityCheck, final boolean oneDot15) {
        ...
        try {
            ...
            for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost))) {
                if (!entity.isDead) {
            ...
            if (!oneDot15) {
                for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2))) {
                    if (!entity.isDead) {
                        if (specialEntityCheck && entity instanceof EntityEnderCrystal) {
                            continue;
                        }
    ...
    public static void placeCrystalOnBlock(final BlockPos pos, final EnumHand hand, final boolean swing, final boolean exactHand, final boolean silent) {
        ...
        final int old = BlockUtil2.mc.player.inventory.currentItem;
