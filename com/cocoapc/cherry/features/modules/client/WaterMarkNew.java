...
{
    public WaterMarkNew() {
        super("SweetPlus", "", Category.CLIENT, true, false, false);
    }
    ...
    public void onRender(final RenderGameOverlayEvent.Post event) {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            final String ping = this.getPing((EntityPlayer)Util.mc.player) + "PING";
            final String fpsText = Minecraft.debugFPS + "FPS ";
            final String server = Minecraft.getMinecraft().isSingleplayer() ? "SinglePlayer".toLowerCase() : Util.mc.getCurrentServerData().serverIP.toLowerCase();
            final String text = "CherryClient || " + server + " || " + ping + " || " + fpsText + " || " + Util.mc.player.getDisplayNameString();
    ...
    private int getPing(final EntityPlayer player) {
        int ping = 0;
        try {
            ping = (int)MathUtil.clamp((float)Objects.requireNonNull(Util.mc.getConnection()).getPlayerInfo(player.getUniqueID()).getResponseTime(), 1.0f, 9999999.0f);
        }
