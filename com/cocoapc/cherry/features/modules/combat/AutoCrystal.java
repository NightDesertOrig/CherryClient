...
public class AutoCrystal extends Module
{
    ...
    public AutoCrystal() {
        super("CA", "Best CA on the market", Category.COMBAT, true, false, false);
        ...
        this.placeDelay = (Setting<Integer>)this.register(new Setting("PlaceDelay", (T)25, (T)(-100), (T)500, v -> this.setting.getValue() == Settings.PLACE && this.place.getValue()));
        ...
        this.breakDelay = (Setting<Integer>)this.register(new Setting("BreakDelay", (T)50, (T)(-100), (T)500, v -> this.setting.getValue() == Settings.BREAK && this.explode.getValue()));
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && Cherry.friendManager.isFriend(friend)) {
    ...
    private boolean isRightThread() {
        return Util.mc.isCallingFromMinecraftThread() || (!Cherry.eventManager2.ticksOngoing() && !this.threadOngoing.get());
    }
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof CherryGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                final Offhand module = Cherry.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
                    ...
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        ...
                        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            ...
                            if (EntityUtill.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue())) {
                                ...
                                if ((this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL) || !Cherry.friendManager.isFriend(player.getName())) {
                                    continue;
                                }
            ...
            if (targettedPlayer != null) {
                ...
                for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    if (EntityUtil.isValid((Entity)player, this.placeRange.getValue() + this.range.getValue())) {
                        ...
                        if ((this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE) || player == null || player.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !Cherry.friendManager.isFriend(player)) {
                            continue;
                        }
        ...
        switch (this.rotate.getValue()) {
            ...
            case ALL: {
                ...
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Cherry.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Cherry.eventManager2.ticksOngoing()) {}
                    ...
                    Cherry.safetyManager.doSafetyCheck();
