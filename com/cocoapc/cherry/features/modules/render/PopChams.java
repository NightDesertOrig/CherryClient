...
public class PopChams extends Module
{
    ...
    public void onPacketReceived(final PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity((World)PopChams.mc.world) != null && (PopChams.self.getValue() || packet.getEntity((World)PopChams.mc.world).getEntityId() != PopChams.mc.player.getEntityId())) {
                final GameProfile profile = new GameProfile(Util.mc.player.getUniqueID(), "");
                (this.player = new EntityOtherPlayerMP((World)Util.mc.world, profile)).copyLocationAndAnglesFrom(packet.getEntity((World)Util.mc.world));
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        if (PopChams.onlyOneEsp.getValue()) {
            if (this.player == null || Util.mc.world == null || Util.mc.player == null) {
                return;
            }
    ...
    public static void renderEntity(final EntityLivingBase entity, final ModelBase modelBase, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        ...
        final float partialTicks = Util.mc.getRenderPartialTicks();
        final double x = entity.posX - Util.mc.getRenderManager().viewerPosX;
        double y = entity.posY - Util.mc.getRenderManager().viewerPosY;
        final double z = entity.posZ - Util.mc.getRenderManager().viewerPosZ;
    ...
    public static void prepareTranslate(final EntityLivingBase entityIn, final double x, final double y, final double z) {
        renderLivingAt(x - Util.mc.getRenderManager().viewerPosX, y - Util.mc.getRenderManager().viewerPosY, z - Util.mc.getRenderManager().viewerPosZ);
    }
