...
public class Tracer extends Module
{
    ...
    public double interpolate(final double now, final double then) {
        return then + (now - then) * Util.mc.getRenderPartialTicks();
    }
    ...
    public float[] getColorByDistance(final Entity entity) {
        if (entity instanceof EntityPlayer && Cherry.friendManager.isFriend(entity.getName())) {
            return new float[] { 0.0f, 0.5f, 1.0f, 1.0f };
        }
        final AutoCrystal autoCrystal = Cherry.moduleManager.getModuleByClass(AutoCrystal.class);
