...
public class AutoBuilder extends Module
{
    ...
    private boolean doSwitch(final boolean back) {
        ...
        switch (this.switchMode.getValue()) {
            ...
            case SILENT: {
                if (i instanceof ItemBlock && (this.allBlocks.getValue() || ((ItemBlock)i).getBlock() instanceof BlockObsidian)) {
                    break;
                }
                if (this.lastSlot == -1) {
                    break;
    ...
    private boolean check() {
        ...
        this.blocksThisTick = 0;
        this.lastSlot = AutoBuilder.mc.player.inventory.currentItem;
        final int blockSlot = this.allBlocks.getValue() ? InventoryUtil.findAnyBlock() : InventoryUtil.findHotbarBlock(BlockObsidian.class);
        this.blockSlot = blockSlot;
        final int n = blockSlot;
        ...
        for (final BlockPos pos : BlockUtil.getSphere(new BlockPos(AutoBuilder.mc.player.posX, Math.ceil(AutoBuilder.mc.player.posY), AutoBuilder.mc.player.posZ).up(), this.range.getValue(), this.range.getValue().intValue(), false, true, 0)) {
            if (!this.placepositions.contains(pos)) {
                ...
                if (this.mode.getValue() == Mode.STAIRS) {
                    switch (this.stairDirection.getValue()) {
                        case NORTH: {
                            if (this.startPos.getZ() - pos.getZ() != pos.getY() - this.startPos.getY()) {
                                continue;
                            }
                            if (Math.abs(pos.getX() - this.startPos.getX()) >= this.width.getValue() / 2) {
                                continue;
                            }
                        ...
                        case EAST: {
                            if (pos.getX() - this.startPos.getX() != pos.getY() - this.startPos.getY()) {
                                continue;
                            }
                            if (Math.abs(pos.getZ() - this.startPos.getZ()) >= this.width.getValue() / 2) {
                                continue;
                            }
                            this.placepositions.add(pos);
                            continue;
                        ...
                        case SOUTH: {
                            if (pos.getZ() - this.startPos.getZ() != pos.getY() - this.startPos.getY()) {
                                continue;
                            }
                            if (Math.abs(this.startPos.getX() - pos.getX()) >= this.width.getValue() / 2) {
                                continue;
                            }
                            this.placepositions.add(pos);
                            continue;
                        ...
                        case WEST: {
                            if (this.startPos.getX() - pos.getX() != pos.getY() - this.startPos.getY()) {
                                continue;
                            }
                            if (Math.abs(this.startPos.getZ() - pos.getZ()) >= this.width.getValue() / 2) {
                                continue;
                            }
                            this.placepositions.add(pos);
                            continue;
